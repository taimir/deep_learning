...
"""
Implementation of an Autoencoder with sigmoid units and a sparsity regularization constraint
for the outputs of the hidden unit.

I implemented this very much differently from the Autoencoder in the Theano tutorial.
I did not corrupt the input, because we already have a regularization term (L1) of other kind here.
Also, in the tutorial the cost was not the sum of squared errors as in here.

As you can see, I kept the same structure as in my neuralnetwork implementation, so that I could reuse the code
for the autoencoder. The only thing that changed is the way the network layers are wired: now the input variable is
the actual target as well.

Finally, I decided not to use shared (tied) weights for the two layers. This is not a requirement for autoencoders,
and my implementation proved that it works fine without this (also in the Theano tutorial it was mentioned that
sharing the parameters is optional).
"""
...
class NeuralLayer:
    def __init__(self, inputs, in_dim, out_dim, activation):
        ...
        :param inputs: a TheanoTensor of the inputs to this layer
...
class NeuralNetClassifier:
    def __init__(self, dim, n_hidden, batch_size=600, max_epochs=150, learning_rate=0.1, sparcity_coef=0.001):
        """

        :param dim: the dimensionality of the feature vectors in the data
        :param n_hidden: number of hidden units
        :param batch_size: for stochastic gradient descend
        :param max_epochs: the model will stop training if this number is reached
        :param learning_rate: step during gradient updates
        :param sparcity_coef: coefficient for the L1 regularization term (lambda)
        """
...
"""
Copied from the Theano tutorial
"""
...
def get_data():
    print("mnist.pkl.gz must be directly in this directory, besides autoenc.py")
